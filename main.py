...
class City(Base):
    ...
    weather_data = Column(JSON)  # Хранит данные о погоде
...
# Функция для обновления прогноза погоды
...
async def add_city(city_name: str, latitude: float, longitude: float, background_tasks: BackgroundTasks, user_id: int):
    with SessionLocal() as db:
        ...
        if user:
            ...
            if not city:
                ...
                raise HTTPException(status_code=400, detail="City is already being tracked")
        else:
...
async def get_weather_by_city_and_time(city_name: str, time: str, user_id: int):
    with SessionLocal() as db:
        user = db.execute(select(User).where(User.id == user_id)).scalars().first()
        if user:
            city = db.execute(select(City).where(City.name == city_name)).scalars().first()
